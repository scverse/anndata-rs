    ...
    ArrayChunk for CsMatI<N, T, u64>
{
        ...
        G: GroupOp<B>,
    {
        ...
        let mut indptr_workspace: Vec<u64> = Vec::new();
        ...
        iter.try_for_each(|mat| {
            ...
            if format == "csc" {
                ...
                indices.extend(0, ArrayView1::from_shape(indices_.len(), indices_)?)?;
            } else if format == "csr" {
                let c = mat.cols();
                if num_cols.is_none() {
                    num_cols = Some(c);
